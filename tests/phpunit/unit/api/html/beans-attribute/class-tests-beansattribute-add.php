<?php
/**
 * Tests for the add() method of Beans_Attribute.
 *
...
class Tests_BeansAttribute_Add extends HTML_Test_Case {
	...
	/**
	 * Test Beans_Attribute::add() should add the attribute when it does not exist in the given attributes.
	 */
	public function test_should_add_the_attribute_when_does_not_exist() {
		...
		foreach ( static::$test_attributes as $beans_id => $markup ) {
			...
			$actual = ( new Beans_Attribute( $beans_id, 'data-test', 'foo' ) )->add( $markup['attributes'] );
	...
	/**
	 * Test Beans_Attribute::add() should add the attribute when an empty array is given.
	 */
	public function test_should_add_the_attribute_when_empty_array_given() {
		...
		foreach ( array_keys( static::$test_attributes ) as $beans_id ) {
			$actual = ( new Beans_Attribute( $beans_id, 'data-test', 'foo' ) )->add( [] );
	...
	/**
	 * Test Beans_Attribute::add() should add the value to an existing attribute's values.
	 */
	public function test_should_add_value_to_existing_attribute_values() {
		...
		foreach ( static::$test_attributes as $beans_id => $markup ) {
			...
			$actual = ( new Beans_Attribute( $beans_id, $name, 'beans-test' ) )->add( $markup['attributes'] );
