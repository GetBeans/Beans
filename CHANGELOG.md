...
* Improved the APIs by adding a thorough unit and integration test suites.
* Improved the APIs by adding type hinting, reducing redundant code, and removing used code.
...
* Improved the Actions API by eliminating storing actions in encoded strings.
...
* Added `beans_array_unique()` to remove duplicate values and re-indexes the array.
* Added `beans_join_arrays_clean()` to join two arrays, removes the duplicate values and empties, and provides an option to re-index the clean array.
* Added `beans_scandir()` to scan the given directory path, lists all the files and directories, and removes the `.` and `..` files.
...
* Added `beans_add_compiler_options_to_settings()` to instantiate `_Beans_Compiler_Options` through a hook instead on file load.
* Added `beans_add_page_assets_compiler()` to instantiate `__Beans_Page_Compiler` through a hook instead on file load.
* Added `beans_add_image_options_to_settings()` to instantiate `___Beans_Image_Options` through a hook instead on file load.
...
* `beans_modify_action()` returns `false` when no hook, callback, priority, and args are not given.
...
* `beans_replace_action()` returns `false` when no hook, callback, priority, and args are not given.
...
* `beans_wrap_markup()` bails out and return false if the given `$tag` is empty.
* `beans_wrap_inner_markup()` bails out and return false if the given `$tag` is empty.
