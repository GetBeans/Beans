...
if ( have_posts() && ! is_404() ) :
	...
	while ( have_posts() ) :
		...
		$article_attributes = array(  // @codingStandardsIgnoreLine - WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound.
		...
		if ( 'post' === get_post_type() ) {
			...
			$article_attributes['itemtype'] = 'http://schema.org/BlogPosting'; // @codingStandardsIgnoreLine - WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound.

			// Only add to blogPost attribute to the main query.
			if ( is_main_query() && ! is_search() ) {
				$article_attributes['itemprop'] = 'blogPost'; // @codingStandardsIgnoreLine - WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound.
			}
		}
